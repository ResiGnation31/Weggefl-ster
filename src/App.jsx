    ...
    <div style={{ minHeight:"100vh", position:"relative", background:T.bg, fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Text',sans-serif", color:T.text, overflowX:"hidden", transition:"background 0.3s, color 0.3s", paddingTop:"env(safe-area-inset-top)" }}>
      ...
      <div style={{ maxWidth:480, margin:"0 auto", padding:"0 16px 100px" }}>
        ...
        <div style={{ display:"inline-flex", background:T.segBg, borderRadius:12, padding:3, gap:2 }}>
          {["sim","real"].map(m => (
            <button key={m} onClick={() => { setGpsMode(m); if(m==="sim") stopGPS(); if(m==="real") setGpsZielOpen(true); }}
          ...
          <div style={{ position:"relative", marginBottom:16 }}>
            <div style={{ position:"relative" }}>
              ...
              <input value={endInput} onChange={e=>onEndInput(e.target.value)}
                placeholder="Ziel eingeben, z.B. Paris"
        ...
        {gpsMode === "real" && (
          <div style={{ marginBottom:16 }}>
            ...
            {(() => {
              ...
              const oderActive = activeIdx === -1;
              return (
                <div style={{ position:"relative", marginBottom:10 }}>
                  {/* — oder — mit Highlight wenn kein Modus aktiv */}
                  <div style={{ textAlign:"center", marginBottom:6, position:"relative", zIndex:1 }}>
                    <span style={{ display:"inline-block", padding:"4px 14px", borderRadius:10, fontSize:".72rem", color: oderActive ? T.accent : T.textMuted, fontWeight: oderActive ? 600 : 400, background: oderActive ? (isDark?"rgba(255,255,255,0.12)":"white") : "transparent", boxShadow: oderActive ? "0 2px 8px rgba(0,0,0,0.10)" : "none", transition:"all 0.4s cubic-bezier(0.4, 0, 0.2, 1)" }}>— oder —</span>
                  </div>
                  {/* Segment Control */}
                  <div style={{ position:"relative", display:"flex", background: isDark?"rgba(255,255,255,0.06)":"rgba(255,255,255,0.7)", borderRadius:14, padding:4 }}>
                    {activeIdx >= 0 && (
                      <div style={{ position:"absolute", top:4, left:`calc(${activeIdx} * (100% - 8px) / 3 + 4px)`, width:"calc((100% - 8px) / 3)", height:"calc(100% - 8px)", background: isDark?"rgba(255,255,255,0.12)":"white", borderRadius:10, boxShadow:"0 2px 8px rgba(0,0,0,0.12)", transition:"left 0.4s cubic-bezier(0.4, 0, 0.2, 1)" }} />
                    )}
                    {modes.map((mode, i) => (
                      <button key={mode} onClick={() => {
                        ...
                        style={{ flex:1, padding:10, background:"transparent", border:"none", borderRadius:10, color: activeIdx===i ? T.accent : T.textMuted, fontSize:13, fontWeight: activeIdx===i ? 600 : 500, cursor:"pointer", fontFamily:"sans-serif", position:"relative", zIndex:1, transition:"color 0.4s cubic-bezier(0.4, 0, 0.2, 1)" }}>
                        {labels[i]}
                      </button>
                    ))}
                  </div>
      ...
      {/* Playbar — fest über Nav-Bar */}
      {(storyTitle || storyLoading || speaking) && (
        <div style={{ position:"fixed", bottom:68, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, zIndex:199, padding:"0 12px" }}>
          <div style={{ background: isDark ? "rgba(26,23,20,0.96)" : "rgba(245,240,232,0.96)", backdropFilter:"blur(20px)", border:`1px solid ${T.border}`, borderRadius:14, padding:"8px 12px" }}>
            {storyLoading ? (
      ...
      {/* Neue Nav-Bar: Home | Tagebuch | Abenteuer | Profil */}
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, background: isDark ? "rgba(26,23,20,0.92)" : "rgba(245,240,232,0.88)", backdropFilter:"blur(20px)", borderTop:"0.5px solid " + T.border, padding:"10px 0 18px", zIndex:200, transition:"background 0.4s" }}>
        <div style={{ display:"flex", width:"100%" }}>
          {[
            ...
            { id:"tagebuch", label:"Tagebuch", svg:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{width:22,height:22}}><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg> },
            { id:"abenteuer", label:"Abenteuer", svg:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{width:22,height:22}}><polygon points="3 11 22 2 13 21 11 13 3 11"/></svg> },
